...
# `all_tf` and `LOOKBACK` which are only defined inside __main__)
# ---------------------------------------------------------------------------
...
def build_data(t):
    """Build the same data dict that build_market_data() produces each live
    scan, including fib_confluence and harmonic keys so strategies #32 and
    #33 score correctly in backtest (previously missing, causing both to
    always score 0/0 silently)."""
    ...
    try:
        data["fib_confluence"] = fib_confluence.compute_confluence(data)
    except Exception:
        data["fib_confluence"] = None
    try:
        data["harmonic"] = harmonic_mod.compute_harmonic_patterns(data)
    except Exception:
        data["harmonic"] = None
    return data
