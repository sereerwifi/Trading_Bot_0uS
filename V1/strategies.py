...
# ----------------------------- 5. London Breakout ------------------------------
def score_london_breakout(data, london_start=(14, 0), london_end=(17, 0)):
    """Builds the first-hour range of the London session (Thai time) from
    M15 bars, then scores a breakout above/below that range during the rest
    of the session."""
    df = data["m15"]
    now = data["now"]
    if len(df) < 10:
...
def score_fibonacci(data):
    ...
    fib50 = swing_high - 0.5 * diff
    fib618 = swing_high - 0.618 * diff

    last, prev = df.iloc[-1], df.iloc[-2]
    zone_low, zone_high = min(fib50, fib618), max(fib50, fib618)
    price = last["close"]
    ...
    if zone_low <= price <= zone_high:
        ...
        if leg_up:
            ...
            note = "price in fib retracement zone of an up-leg"
        else:
            short_score = _clip(base + (20 if macd_cross_down else 0) + (10 if rsi_mid else 0))
            note = "price in fib retracement zone of a down-leg"
    return {"long": long_score, "short": short_score, "note": note}
...
def score_liquidity_sweep(data):
    ...
    if recent_highs:
        ...
        if last["high"] > level and last["close"] < level:
            pierce = (last["high"] - level) / atr_now
            short_score = _clip(40 + pierce * 100)
            note = f"swept liquidity above {level:.2f} then closed back below"
    ...
    if recent_lows:
        ...
        if last["low"] < level and last["close"] > level:
            ...
            if s > long_score:
                long_score = s
                note = f"swept liquidity below {level:.2f} then closed back above"
...
def score_opening_range_breakout(data, session_start=(19, 0)):
    """Same mechanic as London Breakout but anchored to the configured
    overlap/Asia session open instead — first 30 minutes define the range,
    a clean directional break of that range afterward scores the breakout
    direction. Distinguished from score_london_breakout() by using a
    shorter 30-min range and being usable for any session, not just
    London."""
    df = data["m15"]
    now = data["now"]
    if len(df) < 6:
    ...
    if bars_since_open > 8:  # only score the breakout window itself, not all day
...
def score_order_flow_dom(data):
    ...
    long_score = _clip(50 + imbalance * 100) if imbalance > 0 else 0.0
    short_score = _clip(50 - imbalance * 100) if imbalance < 0 else 0.0
...
def score_scalp_london_sweep(data, london_start=(14, 0), london_end=(18, 0),
                              asian_start=(7, 0), asian_end=(12, 0)):
    """Scalping strategy #1 (historically ~55-65% win rate per user research).
    Active only during the Bangkok London-Open window (default 14:00-18:00).
    Builds the Asian session's high/low (07:00-12:00) from M5 bars, then looks
    for a liquidity grab: a wick piercing that Asian high/low which closes
    back inside the range (a fake breakout / stop hunt). The EMA20 vs EMA50
    relationship on M5 is a hard directional filter exactly per spec — EMA20
    above EMA50 allows LONG only, EMA20 below EMA50 allows SHORT only; a sweep
    on the disallowed side is zeroed out rather than scored.
    Suggested risk: SL $5-10, TP $10-20 (min R:R 1:2) — enforce via
    CONFLUENCE_SL_ATR_MULT/CONFLUENCE_TP_RR or your own fixed-$ SL/TP."""
    df = data.get("m5")
    now = data["now"]
